...
AUDIO_UDP_PORT = MACHINES[stream_server_number].audio_udp_port
UPDATE_UDP_PORT = MACHINES[stream_server_number].update_udp_port
...
class Client:
    ...
    def upload_file(self, file_path):
        ...
        self.server_upload_tcp.send(pack_opcode(Operation.UPLOAD))

        file_name = os.path.basename(file_path)
        self.server_upload_tcp.send(pack_num(len(file_name), 16))
        self.server_upload_tcp.send(file_name.encode())
        self.server_upload_tcp.send(pack_num(os.path.getsize(file_path), 32))

        with open(file_path, 'rb') as file_to_send:
            self.server_upload_tcp.sendall(file_to_send.read())
    ...
    def upload_file_flask(self, file):
        ...
        self.server_upload_tcp.send(pack_num(len(data), 32))
        self.server_upload_tcp.sendall(data)
    ...
    def connect_upload(self, machine):
        try:
            ...
            self.server_upload_tcp = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.server_upload_tcp.connect((self.host, self.upload_tcp_port))
    ...
    def check_connection(self):
        ...
        self.connect_upload(connected_servers[0])
        self.connect_stream(random.choice(connected_servers))
